...
fn validate_routing(
    ...
    for entry in collect_config_redirects(root, order, locations, issues) {
        if pending_substitution([&entry.source, &entry.destination]) {
            continue;
        }
        for issue in
            compile_config_redirect(&entry.source, &entry.destination, entry.status, &[]).issues
        {
    ...
    for entry in collect_config_rewrites(root, order, locations, issues) {
        ...
        for issue in compile_config_rewrite(
            &entry.source,
            &entry.destination,
        ...
        if pending_substitution(
            std::iter::once(&entry.source).chain(
                entry
                    .headers
                    .iter()
                    .flat_map(|header| [&header.key, &header.value]),
            ),
        ) {
            continue;
        }
        for issue in compile_config_header(&entry.source, &pairs).issues {
            push_rule_issue(issues, locations, &entry.path, &issue);
        }
    }
}
...
pub fn routing_sections(source: &str) -> (ConfigRouting, Vec<Issue>) {
    ...
    let mut locations = document
        .key_offsets
        .iter()
        .map(|(path, offset)| (path.clone(), locate(source, *offset)))
        .collect::<BTreeMap<_, _>>();
    locations.insert("$".into(), Location { line: 1, column: 1 });
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        for (position, value) in values.iter().enumerate() {
            ...
            let pair = value.as_object().and_then(|value| {
                validate_entry_keys(order, &entry_path, &["key", "value"], locations, issues);
                Some((value.get("key")?.as_str()?, value.get("value")?.as_str()?))
            });
            match pair {
                ...
                None => {
                    ...
                    push_issue(
                        ...
                        "config_invalid",
                        &entry_path,
                        "Header entries must be objects with string key and value.",
                        None,
                    );
                }
            }
        ...
        collected.push(ConfigHeaderRule {
            ...
            headers: pairs
                .into_iter()
                .map(|(key, value)| ConfigHeaderEntry {
                    key: key.to_string(),
...
mod tests {
    ...
    fn published_schema_describes_the_routing_lane() {
        ...
        // are what tells a publisher which one they are writing.
        assert_eq!(
        ...
        assert_eq!(
            schema.pointer("/properties/headers/items/properties/headers/items/required"),
            Some(&json!(["key", "value"]))
        );
    }
    ...
    fn reports_rule_problems_at_the_key_that_caused_them() {
        ...
        assert_eq!((issue.line, issue.column), (5, 7));
    }
