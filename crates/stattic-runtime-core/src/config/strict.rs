...
use crate::protocol::{
    ...
    CONFIG_BUILD_TIMEOUT_MAX_SECONDS, CONFIG_BUILD_TIMEOUT_MIN_SECONDS,
    CONFIG_FILE_MAX_BYTES, CONFIG_META_DESCRIPTION_MAX_CHARS,
    CONFIG_META_IMAGE_MAX_CHARS, CONFIG_META_TITLE_MAX_CHARS, CONFIG_TEMPLATE_LIMIT,
};
use crate::routing::{
...
/// The byte ranges of every substitutable config string. Routing sections allow
/// variables throughout their rule strings; crons allow them only in `path`.
    ...
    document
        ...
        .filter(|(path, _)| {
            ROUTING_SECTIONS
                ...
                || (path.starts_with("$.crons[") && path.ends_with(".path"))
        })
        .map(|(_, span)| *span)
        .collect()
}
...
/// Scheduled requests. The grammar is shared with the current config lane, so
/// a declaration that compiles here compiles at publish and vice versa.
fn validate_crons(
    root: &Map<String, Value>,
    locations: &BTreeMap<String, Location>,
    issues: &mut Vec<Issue>,
) {
    let Some(declared) = root.get("crons") else {
        return;
    };
    for issue in crons::validate(declared).1 {
...
mod tests {
    ...
    /// The grammar and its codes are proven where they live; what the strict
    /// seam adds is that `crons` is a v1 key at all, that its issues address
    /// the entry that carried them, and that both projections keep it verbatim.
    #[test]
    fn carries_crons_through_both_projections_and_addresses_their_issues() {
        let declared = json!([
        ...
        assert_eq!(
            output
                .config
                .and_then(|config| config.get("crons").cloned()),
            Some(declared.clone())
        );
        assert_eq!(
            output
                .effective
                .and_then(|effective| effective.get("crons").cloned()),
            Some(declared)
        );

        let output =
            compile_source(r#"{"version":1,"crons":[{"path":"/x","schedule":"nightly"}]}"#);
    ...
    fn rules_awaiting_substitution_are_not_judged_at_authoring_time() {
        for source in [
            ...
            r#"{"version":1,"crons":[{"path":"/jobs/{{ vars.SLUG }}","schedule":"daily"}]}"#,
        ] {
            let output = compile_source(source);
