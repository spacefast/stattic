...
/// Serving identity for one committed file: exactly what the response compiler
/// and the catalog read. Served headers are NOT baked here — the response
/// table (`responses::file_entry`) is their one compiler, and the engine
/// composes `cache-control` from the entry's cache class at send time.
#[derive(Debug, Clone)]
pub struct FileMeta {
    pub size: u64,
    pub mime: String,
    pub sha256: String,
    /// Content-derived, never a clock reading — see [`content_mtime`].
    pub mtime: u64,
}
...
mod tests {
    ...
    // own copy of preact. The digest directory earns the immutable verdict the
    // response compiler serves by; a path outside it, and markup anywhere,
    // still revalidates.
    #[test]
    fn platform_assets_are_immutable_and_nothing_else_becomes_so() {
        assert!(immutable_path("_spacefast/platform/abc123/preact.js"));
        ...
        assert!(!immutable_path("__spacefast/zero/config.json"));
        assert!(!immutable_path("client.js"));
        assert!(!immutable_path("_spacefast/platform/abc123/index.html"));
    }
}
