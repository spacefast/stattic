...
fn analyze_fields(input: AnalyzeInputFields) -> AnalysisState {
    ...
    // A `sf.jsonc` routing rule needs the same variable a `_redirects` line
    // needs, and only its routing sections do: a marker anywhere else in the
    // document is not substituted, so it is not a requirement either.
...
/// The `sf.jsonc` routing rules, substituted the way `_redirects` text is.
///
...
/// sections happen to hold rule strings, and a value carrying a quote or a
/// backslash would rewrite the document around it. So the pass writes into the
/// routing sections' string literals and nothing else, JSON-escaping each
