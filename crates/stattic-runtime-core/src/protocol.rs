...
///   down to the protected `no` string.
/// - `html` — open: `public, s-maxage=31536000, max-age=0, must-revalidate`,
///   paired with the `A8C-Edge-Cache: cache` opt-in the engine adds at send
///   time. The edge does cache a PHP body, but only with both, so `s-maxage`
///   earns its place; publish purges. Protected: `private, no-cache`.
