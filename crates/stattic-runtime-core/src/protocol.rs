...
/// against authoritative CAS lengths before it creates a single directory, the
/// engine's PHP admission refuses a fresh manifest that declares more, and the
/// control plane preflights the same number. Every other copy of "2 GiB" in the
/// runtime is generated from this one.
pub const VERSION_MAX_TOTAL_BYTES: u64 = 2 * 1024 * 1024 * 1024;
...
/// Set to 1 on the file a version declared as its share-preview image
/// (`meta.image`), when that file is a raster image. A protected Space serves
/// exactly this entry without a session: link previews fetch the image in a
/// separate, cookieless request, so behind the gate a shared link shows no
/// picture. The page and every other path stay gated.
pub const RESPONSE_ENTRY_PREVIEW_IMAGE: &str = "pi";
...
/// Where a rule runs, written on the RULE entries inside the `\0rules` residue
/// rather than on a response entry. Present only on a rule the placement pass
/// gave to the edge, so an artifact compiled with placement off carries the
/// bytes it carried before placement existed.
pub const RESPONSE_ENTRY_PLACEMENT: &str = "placement";
/// The only value [`RESPONSE_ENTRY_PLACEMENT`] takes. A rule that stays at the
/// origin carries no marker at all, and the reason it stayed is finalizer
/// diagnostics, never serving input.
pub const RESPONSE_PLACEMENT_EDGE: &str = "edge";
/// The hostnames the edge was told to answer this version's placed rules on,
/// written once beside the `\0rules` sections. The origin skips a placed rule
/// only for a request on one of these: the scope froze at finalize, so a domain
/// attached afterwards holds none of them and keeps being served from here.
pub const RESPONSE_ENTRY_PLACED_HOSTNAMES: &str = "placedHostnames";
...
fn response_metadata() -> ResponseProtocolMetadata {
    ResponseProtocolMetadata {
        ...
        entry_keys: ResponseEntryKeyMetadata {
            ...
            preview_image: RESPONSE_ENTRY_PREVIEW_IMAGE,
        },
...
pub struct FinalizerLimitMetadata {
    ...
    pub version_max_total_bytes: u64,
    ...
    FinalizerProtocolMetadata {
        ...
        limits: FinalizerLimitMetadata {
            ...
            config_superpowers_override_reason_max_chars:
                CONFIG_SUPERPOWERS_OVERRIDE_REASON_MAX_CHARS,
            version_max_total_bytes: VERSION_MAX_TOTAL_BYTES,
...
pub fn php_source() -> String {
    ...
    let trusted_hosts = php_string_array(crate::egress::TRUSTED_EGRESS_HOSTS);
