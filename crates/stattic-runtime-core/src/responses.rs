...
impl<'a> RuleIndex<'a> {
    fn new(
        ...
        // An unconditional exact redirect with no earlier matching pattern is
        // fully represented by its ordinary response entry. Keeping it in the
        // ordered residue as well only duplicates unreachable policy inside the
        // single `\0rules` entry; a large generated redirect catalog can push
        // that one entry past PHP/opcache's hard per-table ceiling.
