...
impl ResponseEntry {
    ...
    /// A response the serve path resolves from `action` alone: no body, no
    /// compiled headers, never cacheable.
    fn action_only(action: Value) -> Self {
        Self {
            action: Some(action),
            ..Self::bodyless(200, BTreeMap::new())
        }
    }

    /// The 404 a path answers when it exists but must never serve. It carries an
    /// explicit action so it cannot fall through to the SPA fallback, which
    /// would reveal the path by answering differently from a genuine miss.
    fn not_found() -> Self {
        Self {
            action: Some(json!({ "t": RESPONSE_ACTION_NOT_FOUND })),
            ..Self::bodyless(404, BTreeMap::new())
        }
    }
...
///   policy is not the publisher's to pin.
/// - A publisher `Cache-Control` rule otherwise owns the answer (`None` tells
///   the serve path to leave the rule's value alone).
/// - A document never pins; then the framework prefix contract or a bundler's
...
pub(crate) fn compiled_cache_class(
    ...
    publisher_sets_cache_control: bool,
) -> Option<&'static str> {
    if key == THEME_STYLESHEET_URL {
        return Some(CACHE_CLASS_REVALIDATE);
    }
    if publisher_sets_cache_control {
        return None;
    }
    let is_html = match mime_hint {
        Some(mime) => mime.starts_with("text/html"),
        None => {
...
pub(crate) fn carries_content_hash(key: &str) -> bool {
...
pub(crate) fn compile_response_table(
    ...
    for (path, meta) in input.files {
        ...
        // serve their own bytes: a handler is code, not content.
        if input.private.contains(path)
            || is_private_serving_path(path)
            || php_function_route(path).is_some()
        {
            table.insert(request_key(path), ResponseEntry::not_found());
            continue;
        }
    ...
    for (path, meta) in input.files {
        ...
        table.insert(
            route,
            ResponseEntry::action_only(json!({
    ...
    for (path, action) in input.zero_actions {
        table.insert(
            request_key(path),
            ResponseEntry::action_only(action.clone()),
        );
    }
...
impl<'a> RuleIndex<'a> {
    ...
    fn claims(&self, key: &str) -> bool {
        let normalized = normalized_rule_key(key);
    ...
    /// when the artifact is written, so the serve path reads it instead of
...
/// Writes the base table and any route-specific variants, then the root, then
/// the pointer. Nothing reads a table until `root.json` names the root that
/// names it, so a torn publish is invisible rather than half-live. A route table
/// is complete, rather than a sparse overlay, so every alias, fallback, lane and
/// header decision is compiled from the variant's actual bytes.
...
mod tests {
    ...
    fn an_oversized_body_that_needs_a_dropped_header_fails_the_publish() {
        ...
        // it whole.
        publish_response_tables(temp.path(), &entries, &BTreeMap::new(), "0.0.0-test").unwrap();
        ...
        match publish_response_tables(temp.path(), &entries, &BTreeMap::new(), "0.0.0-test") {
    ...
    fn tables_split_by_size_and_the_pointer_is_written_last() {
        ...
        let published =
            publish_response_tables(stage, &small, &BTreeMap::new(), "0.0.0-test").unwrap();
        ...
        let sharded =
            publish_response_tables(stage, &large, &BTreeMap::new(), "0.0.0-test").unwrap();
    ...
    fn a_table_file_above_the_hard_cap_fails_the_publish() {
        ...
        let error = publish_response_tables(temp.path(), &entries, &BTreeMap::new(), "0.0.0-test");
        match error {
    ...
    fn a_large_ordered_rule_catalog_stays_within_the_table_cap() {
        let temp = tempfile::tempdir().unwrap();
        let patterns: Vec<Value> = (0..1_400)
            .map(|index| {
                json!({
                    "source": format!("/legacy-{index}/:splat"),
                    "regex": format!("^/legacy-{index}/(?P<splat>.*)$"),
                    "destination": format!("/docs/archive-{index}/:splat"),
                    "status": 301,
                    "action": "redirect",
                    "order": index,
                    "conditions": {},
        ...
        let published =
            publish_response_tables(temp.path(), &table, &BTreeMap::new(), "0.0.0-test")
                .expect("a valid large routing catalog publishes");

        for name in published.tables.values() {
            assert!(
                std::fs::metadata(temp.path().join(name)).unwrap().len()
                    <= RESPONSE_TABLE_MAX_BYTES as u64
            );
        }
    }
