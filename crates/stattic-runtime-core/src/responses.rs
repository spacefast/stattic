...
impl<'a> RuleIndex<'a> {
    fn new(
        ...
        // An unconditional single-rule exact redirect with no overlapping
        ...
        //
        // Any overlapping pattern keeps the exact rule in the residue, even
        // when that pattern comes later. `claims()` then runs the ordered walk
        // first; dropping the earlier exact would let the later pattern win and
        // invert first-match-wins before the compiled entry can answer.
        let singleton_exact_keys: BTreeSet<String> = redirects_exact
            ...
            .collect();
        let pattern_claimed_exact_keys: BTreeSet<String> = redirects_exact
            .keys()
            .filter(|path| {
                let normalized = normalized_rule_key(path);
                index
                    .redirect_matchers
                    .iter()
                    .any(|(_, regex)| regex.is_match(path) || regex.is_match(&normalized))
            })
            .map(|path| normalized_rule_key(path))
            .collect();
        let compiled_keys: BTreeSet<String> = index
            ...
            .map(|(key, _)| key)
            .filter(|key| {
                singleton_exact_keys.contains(key) && !pattern_claimed_exact_keys.contains(key)
            })
            .collect();
        index
            .redirect_exact_keys
...
mod tests {
    ...
    #[test]
    fn a_later_pattern_rule_keeps_the_earlier_exact_in_the_ordered_residue() {
        let table = compile(
            &[("index.html", b"home")],
            json!({"index": "index.html", "clean_urls": false}),
        ...
        assert!(
            ...
            "the overlapping rules must run in order"
        );
        let redirects = residue(&table, "redirects");
        assert_eq!(
            redirects["exact"]["/promo"][0]["destination"],
            json!("/summer-sale")
        );
        assert_eq!(
