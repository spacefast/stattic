...
/// Statuses a `sf.jsonc` redirect may spell. 200 is a `rewrites` entry and a
/// custom 404 destination stays a `_redirects` feature.
...
/// The status every `sf.jsonc` rewrite compiles to.
...
pub fn compile_config_redirect(
    ...
    compile_config_rule(source, destination, status, None, assigned_hostnames)
}
...
/// The half both config sections share: string hygiene, the rule core, and the
/// cache directive.
...
/// Compile one `sf.jsonc` header entry into the canonical routing rule. The
/// JSON lane sets headers and never removes them.
pub fn compile_config_header(
    ...
    for (index, (key, value)) in entries.iter().enumerate() {
        ...
        match headers::check_header_name(key) {
            headers::HeaderNameVerdict::Accepted {
                ...
                operations.push(HeaderOperation {
                    kind: "set",
                    name: canonical,
    ...
    // A rule with nothing to set is not a rule.
...
pub fn merge_config_routing(
    ...
    for entry in &config.redirects {
        ...
        let compiled = compile_config_redirect(
            &entry.source,
            &entry.destination,
            entry.status,
            assigned_hostnames,
    ...
    for entry in &config.rewrites {
        let deferred = has_pending_variable_destination(&entry.destination);
        let compiled = compile_config_rewrite(
            &entry.source,
            &entry.destination,
    ...
    for entry in &config.headers {
        let entries = entry
            ...
            .collect::<Vec<_>>();
        let compiled = compile_config_header(&entry.source, &entries);
        for issue in &compiled.issues {
            address.diagnostic(
                &mut merged.diagnostics,
