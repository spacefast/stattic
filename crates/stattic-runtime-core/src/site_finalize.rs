...
use crate::finalize::{
    ...
    read_bounded, remove_any, sha256, validate_id, validate_relative_path, write_bytes, write_json,
    write_php, FileMeta, FinalizeError, Result,
};
...
use crate::protocol::{
    ARTIFACT_SCHEMA_VERSION, CONFIG_ACCEPTED_FILES, CONFIG_FILE_MAX_BYTES, TEMPLATE_MAX_BYTES,
...
use crate::responses::{
    compile_response_table, publish_response_tables_with_variants, ResponseCompileInput,
    DENY_ALL_ROBOTS,
};
use crate::storage::{
    apply_templates, blob_root, commit_session_files, install_blob_from, put_blob,
};
...
/// regardless of the pipeline-private set.
...
fn root_readiness_statuses(
    redirects_exact: &Map<String, Value>,
    redirects_pattern: &[Value],
    lookup: &ReadinessLookup<'_>,
) -> Result<Vec<u64>> {
    let mut statuses = BTreeSet::new();
    let mut fallback_reachable = true;
    let rules = readiness_rules_for_path("/", redirects_exact, redirects_pattern)?;

    if let Some(action) = lookup
        .action("/")
        .filter(|action| action.get("action").and_then(Value::as_str) == Some("redirect"))
    {
        let status = action.get("status").and_then(Value::as_u64).unwrap_or(302);
        statuses.insert(status);
        // Private immutable hosts may enter the silent visitor-session
        // acquisition flow before they can challenge or deny the request.
        statuses.insert(302);
        statuses.insert(401);
        statuses.insert(403);
    ...
    for rule in rules {
            ...
            .unwrap_or("redirect")
        {
            ...
            // No public object exists, so an internal rewrite cannot resolve
            // bytes. A notFound route is likewise terminal 404.
            "rewrite" | "notFound" => {
                statuses.insert(404);
            }
            "proxy" => {
    ...
    if fallback_reachable || statuses.is_empty() {
        statuses.insert(404);
    }
    // Access policy is mutable route state, not a finalized-version property.
    // The immutable host can therefore start visitor-session acquisition,
    // challenge, or deny root at any later activation even when the finalized
    // routing result is a redirect/404.
    statuses.insert(302);
    statuses.insert(401);
    statuses.insert(403);
...
impl<'a> ReadinessLookup<'a> {
    fn new(raw: &'a Map<String, Value>) -> Self {
        let encoded = raw
            .iter()
            .map(|(path, action)| (readiness_request_path(path), action))
            .collect();
        Self { raw, encoded }
    }
...
fn public_readiness_statuses(
    ...
    let mut selected_file_reachable = true;
    for rule in rules {
        if !selected_file_reachable {
            break;
        }
            ...
            .unwrap_or("redirect")
        {
            ...
            "rewrite" | "notFound" => {
                ...
                if rule.get("action").and_then(Value::as_str) == Some("notFound") {
                    statuses.insert(404);
                } else {
                    statuses.insert(readiness_rewrite_status(
                        &destination,
                        request_path,
                        lookup,
                        serving_config,
        ...
        if !request_dependent {
            selected_file_reachable = false;
        }
    }
    if selected_file_reachable || statuses.is_empty() {
        statuses.insert(200);
    }
    // Access policy is mutable and the private browser lane redirects through
    // silent session acquisition before it can return a challenge or denial.
    statuses.insert(302);
    statuses.insert(401);
    statuses.insert(403);
    Ok(statuses.into_iter().collect())
...
fn runtime_readiness_target(
    ...
    if public_files.is_empty() {
        ...
        return Ok(json!({
            "path":"/",
            "expected_statuses":root_readiness_statuses(
                redirects_exact,
                redirects_pattern,
                lookup,
            )?
    ...
    Ok(json!({
        "path":request_path,
        "expected_statuses":public_readiness_statuses(
            &request_path, redirects_exact, redirects_pattern, lookup, serving_config, fallback
        )?
    }))
}
...
fn run_finalize_pipeline(
    ...
    let mut convention_map = convention_files(stage_root)?;
    for (key, value) in [
        (
            "redirects",
    ...
    // Everything the pipeline produced lands in the CAS: the tables address
    ...
    let originals = original_objects(stage_root, &blobs, files)?;
    let variant_files =
    ...
    let table = compile_response_table(&ResponseCompileInput {
        files,
        private: &private,
        serving_config: &serving_config,
        redirects_exact: &redirects_exact,
        ...
        noindex_host: noindex_host(&serving)?,
    });
    let mut route_tables = BTreeMap::new();
    for (route_name, variant_files) in &variant_files {
        route_tables.insert(
            route_name.clone(),
    ...
    let published = publish_response_tables_with_variants(
        stage_root,
        &table,
        &route_tables,
...
fn finalize_catalog_delta(
    ...
    let previous_root = private_root
        ...
        .join("versions")
        .join(previous_id);
    // A version that predates the catalog cannot be diffed. That is a missing
    // migration, and the caller answers it by purging host-wide — never by
    // reconstructing the previous file set from somewhere else.
    let Some(previous) = read_version_catalog(&previous_root)? else {
        return Ok(None);
    };
...
fn resolve_template_substitution(
    ...
    let variable_scopes: Vec<VariableScope> = match input.body.get("variable_scopes") {
        None | Some(Value::Null) => Vec::new(),
        Some(scopes) => {
            serde_json::from_value(scopes.clone()).map_err(|error| FinalizeError::Invalid {
                code: "invalid_variable_scopes",
                message: format!("variable_scopes is not a resolved variable scope list: {error}"),
                details: None,
            })?
        }
    };
    let system_variables: BTreeMap<String, String> = match input.body.get("system_variables") {
        None | Some(Value::Null) => BTreeMap::new(),
        Some(value) => {
            serde_json::from_value(value.clone()).map_err(|error| FinalizeError::Invalid {
                code: "invalid_system_variables",
                message: format!("system_variables is not a name/value map: {error}"),
                details: None,
            })?
        }
    };
    let channels: Vec<Channel> = match input.body.get("channels") {
        None | Some(Value::Null) => Vec::new(),
        Some(value) => {
            serde_json::from_value(value.clone()).map_err(|error| FinalizeError::Invalid {
                code: "invalid_channels",
                message: format!("channels is not a channel list: {error}"),
                details: None,
            })?
        }
    };
    ...
    let analysis_without_templates = AnalyzeInputFields {
    ...
    let declared = analyze(AnalyzeInput {
        ...
        config_candidates: analysis_without_templates.config_candidates.clone(),
        convention_files: analysis_without_templates.convention_files.clone(),
        routing_config: analysis_without_templates.routing_config.clone(),
        template_sources: BTreeMap::new(),
    });
...
fn platform_csp_sources(serving: &Map<String, Value>) -> Result<PlatformCspSources> {
    match serving.get("platform_csp_sources") {
        None | Some(Value::Null) => Ok(PlatformCspSources::new()),
        Some(value) => {
            serde_json::from_value(value.clone()).map_err(|error| FinalizeError::Invalid {
                code: "invalid_platform_csp_sources",
                message: format!(
                    "platform_csp_sources is not a directive-to-source-list map: {error}"
                ),
                details: None,
            })
        }
...
mod tests {
    ...
    fn staged_config_rules_merge_behind_the_staged_convention_files() {
        ...
        let table = finalized_table(&private);
        // Both rules for /old are published; the file rule is asked first.
        let file_rule = table.find("/file-wins").expect("file rule published");
        let config_rule = table.find("/config-loses").expect("config rule published");
        assert!(file_rule < config_rule);
        assert!(table.contains("/archive/"));

        // No `_headers` value is compiled into an entry. The
        // rule ships whole in the residue — matcher, order and templated value
        // intact — and PHP applies it against the URL the visitor asked for.
        // An entry's header map is lowercased, so the lowercase spelling
        // appearing anywhere would mean a value got baked back in.
        assert!(!table.contains("x-config"), "{table}");
        assert!(table.contains("'name' => 'X-Config'"), "{table}");
    }
...
/gone/* /blog/404.html 404",
    ...
    fn zero_all_features_fixture_finalizes_to_invocable_private_artifacts() {
        ...
        let output = finalize_site(input).expect("fixture finalizes");
    ...
    fn a_replay_answers_for_a_version_whose_files_were_tiered_out_of_the_tree() {
        ...
        let published = finalize_site(input.clone()).unwrap();
        ...
        let replayed = finalize_site(input).unwrap();
    ...
    fn finalize_reports_the_catalog_delta_against_the_previous_version() {
        ...
        let published = finalize_site(next.clone()).unwrap();
        ...
        let replayed = finalize_site(next).unwrap();
    ...
    fn a_changed_serving_state_forces_a_host_wide_purge() {
        ...
        let delta = finalize_site(next).unwrap().delta.expect("a delta");
