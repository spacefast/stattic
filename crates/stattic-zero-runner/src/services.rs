...
use crate::db::DbError as ServiceError;
...
fn execute_service_frame(raw: &str) -> Result<Value, ServiceError> {
    ...
    let frame: ServiceFrame = serde_json::from_str(raw).map_err(|_| {
        ServiceError::new("service_payload_invalid", "The service frame is not valid.")
    })?;
...
fn spam_check(config: &ServiceConfig, payload: &Map<String, Value>) -> Result<Value, ServiceError> {
...
fn akismet_call(
    ...
    agent()
        ...
        .map_err(|error| match error {
            ureq::Error::StatusCode(status) => ServiceError::new(
                "service_upstream_refused",
...
fn email_send(config: &ServiceConfig, payload: &Map<String, Value>) -> Result<Value, ServiceError> {
    ...
    let normalized = serde_json::to_string(&Value::Object(payload.clone())).map_err(|_| {
        ServiceError::new("email_message_invalid", "The message could not be encoded.")
    })?;
    ...
    .map_err(|error| ServiceError::new("email_outbox_unavailable", error))?;
...
fn required_str<'a>(payload: &'a Map<String, Value>, field: &str) -> Result<&'a str, ServiceError> {
    payload
        .get(field)
        .and_then(Value::as_str)
        .filter(|value| !value.is_empty())
...
fn read_body(response: ureq::http::Response<ureq::Body>) -> Result<String, ServiceError> {
