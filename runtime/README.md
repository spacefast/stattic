...
1. Run `installer.php` against an engine ZIP. It stores the complete engine under an immutable
   ...
   in `.stattic/active-release.php` — a one-statement PHP file, so every request resolves the
   release from opcache instead of reading it.
...
```text
htdocs/.stattic/active-release.php          atomic relative path to the active release
