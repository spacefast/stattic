...
1. `POST /__spacefast/api.php?route=/spaces/{spaceId}/versions` -> returns `upload_id` for a
   declared file manifest. Local-byte entries require `sha256`; URL-sourced entries may omit it.
2. Upload bytes with the session-scoped upload JWT
   ...
   - local bytes: `PUT /__spacefast/upload.php?route=/spaces/{spaceId}/blobs/{sha256}`
