...
(static function (): void {
    ...
    if (!is_string($releaseRoot)) {
        // The pointer is a PHP artifact, so the read every request pays for is
        // an opcache hit instead of an open + read + trim of a data file.
        $target = @include $installRoot . '/active-release.php';
        if (!is_string($target)) {
            // One release of tolerance: a box whose last activation predates
            // the PHP pointer still carries only the plain-text file. Delete
            // this fallback in the release after the one that ships it — the
            // installer stops writing the plain file on its first converge.
