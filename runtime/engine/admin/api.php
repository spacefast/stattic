...
function _stattic_runtime_admin_api(string $privateRoot, string $method, string $requestPath, string $surface): void
{
    ...
    foreach (_stattic_runtime_admin_routes($privateRoot, $surface) as $route) {
...
function _stattic_runtime_admin_routes(string $privateRoot, string $surface): array
{
    if ($surface === 'upload') {
...
function _stattic_runtime_admin_management_routes(): array
{
    $rows = [
        ...
        ['POST', '#^/probe/headers$#', 'probe_headers', [], 'none', true, '_stattic_runtime_probe_headers_route'],
        ...
        ['POST', '#^/spaces/([^/]+)/versions$#', 'create_version', ['space_id' => 1], 'none', false, '_stattic_runtime_create_version'],
        ...
        ['POST', '#^/spaces/([^/]+)/pins$#', 'create_pin', ['space_id' => 1], 'space', false, '_stattic_runtime_create_space_pin'],
        ['DELETE', '#^/spaces/([^/]+)/pins/([^/]+)$#', 'release_pin', ['space_id' => 1, 'pin_id' => 2], 'space', false, '_stattic_runtime_delete_space_pin'],
...
function _stattic_runtime_admin_upload_routes(string $privateRoot): array
{
    return [
        [
            'lane' => 'upload',
            'methods' => ['POST'],
            'pattern' => '#^/spaces/([^/]+)/blobs/have$#',
            'binary' => false,
            'method_mismatch' => [
                'allow' => 'POST',
                'code' => 'runtime_upload_operation_not_supported',
                'message' => 'Blob negotiation only supports POST.',
            ],
            'handler' => static fn (array $matches, array $claims) => _stattic_runtime_upload_blobs_have($privateRoot, $matches[1], $claims),
        ],
        [
            'lane' => 'upload',
            'methods' => ['PUT'],
            'pattern' => '#^/spaces/([^/]+)/blobs/([^/]+)$#',
            'binary' => true,
            'method_mismatch' => [
                'allow' => 'PUT',
                'code' => 'runtime_upload_operation_not_supported',
                'message' => 'Blob upload only supports PUT.',
            ],
            'handler' => static fn (array $matches, array $claims) => _stattic_runtime_upload_blob($privateRoot, $matches[1], $matches[2], $claims),
        ],
        [
            'lane' => 'upload',
            'methods' => ['POST'],
