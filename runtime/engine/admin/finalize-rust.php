...
/**
 * Every path the reusable version can hand forward, as retained-file entries.
 *
 * The SOURCE view: retention republishes the uploaded object, and the finalizer
 * re-runs substitution over it. Taking the served byte instead would bake the
 * previous publish's variable values into the new version.
 *
 * @return list<array{path: string, sha256: string, size: int, contentType: string}>
 */
function _stattic_runtime_retained_files_from_catalog(array $catalog): array
{
    $entries = [];
    foreach ($catalog['paths'] as $path => $entry) {
        ...
        $source = _stattic_runtime_catalog_object($entry['source'] ?? null);
        if ($source === null) {
            continue;
        }
        $entries[] = [
...
function _stattic_runtime_prepare_retained_blobs(string $privateRoot, string $spaceId, array $session): array
{
    ...
    $retainAll = ($session['retain_all'] ?? false) === true && $entries === [];
    if ($entries === [] && !$retainAll) {
        return $session;
    }
    ...
    if ($retainAll) {
        // "Retain everything from version X" — the caller sends no list, the
        // runtime materializes it from the catalog it already owns.
        $entries = _stattic_runtime_retained_files_from_catalog($catalog);
    }
