...
{
    $previousIntent = _stattic_runtime_read_json(_stattic_space_root($privateRoot, $spaceId) . '/hostname-intent.json');
...
// this is the read half of a read-modify-write.
function _stattic_runtime_read_route_pointer(string $privateRoot): ?array
{
...
function _stattic_runtime_read_route_owners(string $privateRoot, ?array $pointer): ?array
{
    ...
    $loaded = @include $privateRoot . '/routes/' . $pointer['owners'];
    return is_array($loaded) && is_array($loaded['owners'] ?? null) ? $loaded['owners'] : null;
}

// Any unexpected on-disk shape returns null — the caller falls back to a full rebuild.
...
{
    $stored = _stattic_runtime_read_json(_stattic_space_root($privateRoot, $spaceId) . '/entitlements.json');
...
function _stattic_runtime_resolve_route_target(string $privateRoot, string $spaceId, array $target, array &$pointerCache = []): ?array
{
    ...
    if ($type === 'version') {
        ...
        $production = _stattic_runtime_read_json(_stattic_route_pointer_path($privateRoot, $spaceId, 'production'));
    ...
    if ($type === 'route') {
        ...
        $pointer = _stattic_runtime_read_json(_stattic_route_pointer_path($privateRoot, $spaceId, $routeName));
...
// or pruned by age. `gen` is a monotonically increasing int that participates in
// the serve path's memo key, so every write invalidates every stale memo by
// making its key unreachable.
...
function _stattic_runtime_route_shard_gc(string $privateRoot): int
{
    $named = _stattic_runtime_referenced_route_shards([
        _stattic_runtime_read_json($privateRoot . '/routes/current.json'),
        _stattic_runtime_read_json($privateRoot . '/routes/previous.json'),
    ]);
...
function _stattic_runtime_sync_space_overlay(string $privateRoot, string $spaceId): void
{
    ...
    foreach (glob($spaceRoot . '/routes/*.json') ?: [] as $pointerPath) {
        $pointer = _stattic_runtime_read_json((string) $pointerPath);
        if (
            is_array($pointer)
    ...
    $liveMetadata = is_string($liveVersionId)
        ? _stattic_runtime_read_json(_stattic_version_root($privateRoot, $spaceId, $liveVersionId) . '/metadata.json')
        : null;
...
function _stattic_runtime_write_space_overlay(string $privateRoot, string $spaceId, array $overlay): void
{
    ...
    $current = _stattic_runtime_read_json($pointerPath);
