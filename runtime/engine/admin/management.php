...
function _stattic_runtime_create_version(string $privateRoot, string $spaceId, array $claims): void
{
    ...
    if ($retainedFiles !== [] && $reusableVersionId === null) {
        _stattic_problem_response(422, 'reusable_version_required', 'Retained files require a reusable version.');
    }
    // `reusable_version_id` with NO list is the retain-everything signal: the
    // runtime owns the prior version's catalog, so the caller has nothing to
    // enumerate and nothing to get wrong. The list is materialized at finalize,
    // under the space lock, because reading a prior catalog here — this route is
    // lock=none, and deliberately so, to keep the lazy publish path free of a
    // pre-upload round trip — would race finalize and the GC.
    $retainAll = $reusableVersionId !== null && $retainedFiles === [];
...
function _stattic_runtime_finalize_version(string $privateRoot, string $spaceId, string $versionId, array $claims): void
{
    ...
    if (($session['lazy'] ?? false) === true && is_array($body['session'] ?? null)) {
        ...
        if (($descriptor['upload_id'] ?? null) === $uploadId && ($descriptor['space_id'] ?? null) === $spaceId && ($descriptor['version_id'] ?? null) === $versionId) {
            ...
            $session['retain_all'] = ($session['reusable_version_id'] ?? null) !== null
                && ($session['retained_files'] ?? []) === [];
            $session = _stattic_runtime_publish_session_replace($privateRoot, $spaceId, $uploadId, static function (array $current) use ($session): array {
                ...
                $current['retain_all'] = $session['retain_all'];
                return $current;
            });
        }
    }
 ...
 */
function _stattic_runtime_finalize_ready_response(string $privateRoot, string $spaceId, string $versionId, string $versionRoot, ?string $activationEventId = null, ?array $metadata = null, ?array $purge = null): never
{
...
function _stattic_runtime_journal_cursor_from_token(string $token): ?array
{
    ...
    $cursor = is_string($decoded) ? json_decode($decoded, true) : null;
...
function _stattic_runtime_write_route_pointer(string $privateRoot, string $spaceId, string $routeName, string $versionId, array $body, array $claims, bool $storeIntent): array
{
    ...
    if (is_array($previousRoute)) {
        ...
        $routeEvent['previous_public_exposure'] =
            _stattic_runtime_public_exposure_descriptor($previousConfig);
    ...
    // it: that record is a promise the activation is already SERVING.
    _stattic_runtime_update_route_index($privateRoot, $spaceId);
    $activationEventId = _stattic_runtime_record_management_event($privateRoot, $claims, $routeEvent);
        ...
        if ($raw['authorization'] !== null && $authorization === null) {
            _stattic_json_response(422, [
                'error' => [
                    ...
                    'message' => 'Authorization projection is invalid.',
                ],
            ]);
        }
...
{
    return rtrim(strtr(base64_encode($sortKey), '+/', '-_'), '=');
}
...
// A pin is the GC's own contract — admin/tier.php refuses to delete any sha a
// live pin names. `mode: snapshot` freezes what named versions plus the uploads
// store reference (collected with the GC's OWN collectors, so a pin can never be
// narrower than the live set); `mode: shas` names the set outright. `expires_at`
// is the hard backstop: the GC treats an expired pin as absent.
const STATTIC_RUNTIME_MOVE_PIN_TTL_MAX_SECONDS = 86400;
const STATTIC_RUNTIME_MOVE_PIN_SHAS_MAX = 100000;

function _stattic_runtime_create_space_pin(string $privateRoot, string $spaceId, array $claims): void
{
    if (!is_dir(_stattic_space_root($privateRoot, $spaceId))) {
        _stattic_problem_response(404, 'space_not_found', 'Space not found.');
    }
    $operationId = is_string($claims['operation_id'] ?? null) ? trim($claims['operation_id']) : '';
    if ($operationId === '') {
        _stattic_problem_response(403, 'runtime_operation_missing', 'Runtime management token requires an operation id.');
    }
    $body = _stattic_json_body();
    $ttlSeconds = $body['ttl_seconds'] ?? null;
    if (!is_int($ttlSeconds) || $ttlSeconds < 1 || $ttlSeconds > STATTIC_RUNTIME_MOVE_PIN_TTL_MAX_SECONDS) {
        _stattic_problem_response(422, 'runtime_pin_request_invalid', 'ttl_seconds must be a positive integer of at most 86400.');
    }
    $mode = $body['mode'] ?? null;
    if ($mode !== 'snapshot' && $mode !== 'shas') {
        _stattic_problem_response(422, 'runtime_pin_request_invalid', 'mode must be "snapshot" or "shas".');
    }
    $shas = $mode === 'snapshot'
        ? _stattic_runtime_pin_snapshot_shas($privateRoot, $spaceId, $body)
        : _stattic_runtime_pin_explicit_shas($body);

    // Keyed by operation id, so a retried create converges on the same pin
    // instead of stacking a second one the mover would have to release twice.
    $pinId = 'move-' . _stattic_runtime_id($operationId, 'operation_id');
    $expiresAt = gmdate('c', time() + $ttlSeconds);
    $store = _stattic_runtime_publish_pins_store($privateRoot, $spaceId);
    _stattic_record_store_ensure($store);
    _stattic_record_store_put($store, $pinId, [
        'shas' => $shas,
        'expires_at' => $expiresAt,
    ]);
    _stattic_runtime_record_management_event($privateRoot, $claims, [
        'event' => 'space_pin_created',
        'space_id' => $spaceId,
        'pin_id' => $pinId,
        'mode' => $mode,
        'sha_count' => count($shas),
    ]);
    _stattic_json_response(201, [
        'space_id' => $spaceId,
        'pin_id' => $pinId,
...
/** @return list<string> */
function _stattic_runtime_pin_snapshot_shas(string $privateRoot, string $spaceId, array $body): array
{
    if (!function_exists('_stattic_tier_collect_shas_from_json')) {
        _stattic_problem_response(500, 'runtime_pin_unsupported', 'This engine cannot enumerate the live blob set.');
    }
    $versionIds = $body['version_ids'] ?? null;
    if (!is_array($versionIds) || !array_is_list($versionIds)) {
        _stattic_problem_response(422, 'runtime_pin_request_invalid', 'version_ids must be a list of version ids.');
    }
    $includeUploads = array_key_exists('include_uploads', $body) ? $body['include_uploads'] : true;
    if (!is_bool($includeUploads)) {
        _stattic_problem_response(422, 'runtime_pin_request_invalid', 'include_uploads must be a boolean.');
    }
    ...
    foreach ($versionIds as $versionId) {
        ...
        $versionRoot = _stattic_version_root($privateRoot, $spaceId, _stattic_runtime_id($versionId, 'version_id'));
        if (!is_dir($versionRoot)) {
            _stattic_problem_response(404, 'version_not_found', 'Version not found.', ['details' => ['version_id' => $versionId]]);
        }
        // The collector returns the declaration's TIMESTAMP; null — not falsy —
        // is its "unreadable" answer, and a 0 timestamp is a readable version.
        $publishedAt = _stattic_tier_collect_shas_from_json($versionRoot . '/metadata.json', $shas);
        if ($publishedAt === null) {
            // Same refusal the GC makes on an unreadable declaration: a pin that
            // guessed the set would be worse than no pin at all.
            _stattic_problem_response(409, 'runtime_pin_snapshot_unavailable', 'The live blob set cannot be read right now.');
        }
        foreach (glob($versionRoot . '/responses-*.php') ?: [] as $table) {
            if (is_string($table)) {
                _stattic_tier_collect_shas_from_artifact($table, $shas, $publishedAt);
            }
        }
    }
    if ($includeUploads) {
        foreach (_stattic_record_store_records(_stattic_uploads_store($privateRoot, $spaceId)) as $raw) {
            $record = _stattic_uploads_record($raw);
            if ($record !== null) {
                $shas[$record['sha256']] = true;
            }
        }
    }
...
/** @return list<string> */
function _stattic_runtime_pin_explicit_shas(array $body): array
{
    $raw = $body['shas'] ?? null;
    if (!is_array($raw) || !array_is_list($raw) || count($raw) > STATTIC_RUNTIME_MOVE_PIN_SHAS_MAX) {
        _stattic_problem_response(422, 'runtime_pin_request_invalid', 'shas must be a list of at most 100000 sha256 hex digests.');
    }
    $shas = [];
    foreach ($raw as $sha) {
...
function _stattic_runtime_delete_space_pin(string $privateRoot, string $spaceId, string $pinId, array $claims): void
{
    $store = _stattic_runtime_publish_pins_store($privateRoot, $spaceId);
    // `released` reports whether THIS call dropped the pin, so a retrying mover
    // can tell an absent pin from one it just freed.
    $released = _stattic_record_store_get($store, $pinId) !== null;
    _stattic_record_store_delete($store, $pinId);
    _stattic_runtime_record_management_event($privateRoot, $claims, [
        'event' => 'space_pin_released',
        'space_id' => $spaceId,
        'pin_id' => $pinId,
        'released' => $released,
    ]);
    _stattic_json_response(200, ['space_id' => $spaceId, 'pin_id' => $pinId, 'released' => $released]);
}

/**
 * Every header class a compiled entry can carry. The probe MEASURES which
 * survive X-Accel-Redirect rather than believing the split below, and is
 * re-measured after each provider change.
 */
const STATTIC_RUNTIME_HEADER_PROBE_HEADERS = [
    // Survive the accel hop today.
    'Content-Type' => 'text/plain; charset=utf-8',
    'Cache-Control' => 'private, no-store',
    'Content-Disposition' => 'inline; filename="probe.txt"',
    'Expires' => 'Thu, 01 Jan 1970 00:00:00 GMT',
    'Set-Cookie' => 'sf_probe=1; Path=/__spacefast; HttpOnly; SameSite=Lax',
    // Dropped by the accel hop today — the reason lane=1 exists (D130/D131).
    'Content-Language' => 'en',
    'ETag' => '"sfprobe-1"',
    'Last-Modified' => 'Thu, 01 Jan 1970 00:00:00 GMT',
    'Vary' => 'Accept-Encoding',
    'Link' => '</__spacefast/probe>; rel="canonical"',
    'X-Content-Type-Options' => 'nosniff',
    'X-Frame-Options' => 'SAMEORIGIN',
    'Referrer-Policy' => 'strict-origin-when-cross-origin',
    'Content-Security-Policy' => "default-src 'none'",
    // Strict-Transport-Security is deliberately ABSENT: contracts §16 measured it
    // as provider-owned (rewritten to a strong value on the way out), so the
    // runtime never emits it — not even to measure a hop whose answer is known.
    'Permissions-Policy' => 'interest-cohort=()',
    'Cross-Origin-Opener-Policy' => 'same-origin',
    'Cross-Origin-Resource-Policy' => 'same-origin',
    'Access-Control-Allow-Origin' => 'https://spacefast.com',
    'X-Spacefast-Probe' => 'header-conformance',
];

/**
 * CALLER CONTRACT: the response BODY is the JSON list of header names the
 * runtime emitted — in the body precisely because a header cannot report on
 * headers being stripped. The caller diffs it against the headers it actually
 * received; that difference is the measurement.
 */
function _stattic_runtime_probe_headers_route(string $privateRoot, array $claims): void
{
    $body = (string) json_encode(
        ['probe' => 'header-conformance', 'sent' => array_keys(STATTIC_RUNTIME_HEADER_PROBE_HEADERS)],
        JSON_UNESCAPED_SLASHES
    ) . "\n";

    $probeDir = $privateRoot . '/runtime/probe';
    _stattic_runtime_mkdir($probeDir);
    $probePath = $probeDir . '/headers-' . substr(hash('sha256', $body), 0, 16) . '.txt';
    _stattic_runtime_assert_private_path($probePath);
    if (!is_file($probePath)) {
        $tmp = $probePath . '.tmp-' . bin2hex(random_bytes(6));
        if (@file_put_contents($tmp, $body) !== strlen($body) || !@rename($tmp, $probePath)) {
            @unlink($tmp);
            _stattic_problem_response(500, 'runtime_header_probe_failed', 'The header probe blob could not be written.');
        }
    }

    $headers = STATTIC_RUNTIME_HEADER_PROBE_HEADERS;
    _stattic_send_server_file($probePath, $privateRoot, $headers);
    _stattic_response_send(200, $body, '', $headers);
}
