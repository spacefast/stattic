...
function _stattic_tier_collect_shas_from_artifact(string $path, array &$shas, int $at): void
{
    $source = @file_get_contents($path, false, null, 0, 4194304);
    if (!is_string($source) || preg_match_all('/[a-f0-9]{64}/', $source, $matches) < 1) {
        return;
    }
 ...
 * flight.
 *
 * Null means "this space cannot be reasoned about right now" — an unreadable
 * version declaration — and the caller then touches nothing in it.
 *
...
function _stattic_tier_space_live_shas(string $privateRoot, string $spaceId): ?array
{
    ...
    $shas = [];
    foreach (glob($spaceRoot . '/versions/*', GLOB_ONLYDIR) ?: [] as $versionRoot) {
        if (!is_string($versionRoot)) {
            continue;
        }
    ...
    if ($overlayName !== null && strpos($overlayName, '..') === false) {
        ...
        if (is_file($overlayPath)) {
            _stattic_tier_collect_shas_from_artifact(
                $overlayPath,
                $shas,
    ...
    foreach ([
        $spaceRoot . '/uploads/*.json',
        $spaceRoot . '/uploads/*/*.json',
        $spaceRoot . '/publish-sessions/*.json',
    ] as $pattern) {
        foreach (glob($pattern) ?: [] as $path) {
            if (is_string($path)) {
                _stattic_tier_collect_shas_from_json($path, $shas);
            }
        }
    }
    return $shas;
...
function _stattic_tier_space_pinned_shas(string $privateRoot, string $spaceId, int $now): ?array
{
    $spaceRoot = _stattic_space_root($privateRoot, $spaceId);
    $pinned = [];
    foreach (glob($spaceRoot . '/pins/*.json') ?: [] as $path) {
        if (!is_string($path)) {
            continue;
        }
...
/**
 * One space's CAS, as {sha => ['path', 'stat', 'demoted_at'|null]}. `stat` is
 * null when the body is gone but its demote mark survives — the tombstone that
 * says "these bytes are in S3 and only a promote brings them back".
 *
 * @return array<string, array{path: string, stat: array|null, demoted_at: int|null}>|null
 */
function _stattic_tier_space_blobs(string $privateRoot, string $spaceId): ?array
{
    ...
    $prefixes = @scandir($blobsRoot);
    if (!is_array($prefixes)) {
        return null;
    }
    $blobs = [];
    foreach ($prefixes as $prefix) {
        if (preg_match('/^[a-f0-9]{2}$/', $prefix) !== 1 || !is_dir($blobsRoot . '/' . $prefix)) {
            continue;
        }
        ...
        foreach ($entries as $entry) {
            ...
            $blobs[$sha] ??= ['path' => $path, 'stat' => null, 'demoted_at' => null];
            if ($isMark) {
...
// Deleting is a two-tick decision on purpose: a blob is only ever removed after
// it was ALREADY a candidate one grace period ago, so a publish that declares a
// blob between two ticks cannot lose it to a scan that started earlier.
function _stattic_tier_gc_update_marks(string $privateRoot, string $spaceId, array $blobs, array $live, int $now): array
{
    $path = _stattic_tier_gc_marks_path($privateRoot, $spaceId);
    $stored = _stattic_runtime_read_json($path);
    $marks = [];
    if (is_array($stored)) {
        foreach ($stored as $sha => $firstSeen) {
            // Drop marks for blobs that became live again or left the disk;
            // either way their previous candidacy is void.
            if (is_string($sha) && is_int($firstSeen) && isset($blobs[$sha]) && !isset($live[$sha])) {
                $marks[$sha] = $firstSeen;
            }
        }
    }
    foreach ($blobs as $sha => $blob) {
        if (!isset($live[$sha]) && !isset($marks[$sha])) {
            $marks[$sha] = $now;
        }
    }
    $storedMarks = is_array($stored) ? $stored : [];
    if ($marks !== $storedMarks && !($marks === [] && !is_array($stored))) {
        _stattic_runtime_write_json_atomic($path, $marks);
    }
    return $marks;
}
...
function _stattic_tier_space_blob_gc(string $privateRoot, string $spaceId, int $now, int $grace): array
{
    ...
    $blobs = _stattic_tier_space_blobs($privateRoot, $spaceId);
    if ($blobs === null) {
        return $skipped;
    }
    ...
    $marks = _stattic_tier_gc_update_marks($privateRoot, $spaceId, $blobs, $live, $now);
    ...
    foreach ($blobs as $sha => $blob) {
        ...
        if ($blob['stat'] === null) {
            ...
            // tombstone is the evidence S3 holds these bytes; for one nothing
            // declares any more it is residue.
            if (!isset($live[$sha]) && ($marks[$sha] ?? $now) <= $undeclaredDeadline) {
                $deletions[] = [$sha, $blob['path'], true, 0];
            }
            continue;
        }
        $size = (int) ($blob['stat']['size'] ?? 0);
        if (isset($live[$sha])) {
            ...
            if (
                ...
                $deletions[] = [$sha, $blob['path'], false, $size];
            }
            continue;
        }
        ...
        if (($marks[$sha] ?? $now) <= $undeclaredDeadline) {
            $deletions[] = [$sha, $blob['path'], true, $size];
        }
    }

    $applied = _stattic_tier_gc_apply_deletions($privateRoot, $spaceId, $deletions);
    $deleted = [...$applied['collected'], ...$applied['evicted']];
    if ($deleted === []) {
        return ['complete' => $applied['complete'], 'deleted' => 0, 'bytes' => 0];
    }
    _stattic_runtime_append_journal($privateRoot, [
        'event' => 'local_blob_gc',
        'space_id' => $spaceId,
    ...
    foreach ($deleted as $sha) {
        unset($remainingMarks[$sha]);
    }
    _stattic_runtime_write_json_atomic(
        _stattic_tier_gc_marks_path($privateRoot, $spaceId),
        $remainingMarks
    );
    return ['complete' => $applied['complete'], 'deleted' => count($deleted), 'bytes' => $applied['bytes']];
}
...
function _stattic_tier_space_ids(string $privateRoot): array|false
{
    ...
    $entries = @scandir($spacesRoot);
    if (!is_array($entries)) {
        return false;
    }
    $spaceIds = [];
    foreach ($entries as $entry) {
        if ($entry !== '.' && $entry !== '..' && _stattic_runtime_id_valid($entry) && is_dir($spacesRoot . '/' . $entry)) {
            $spaceIds[] = $entry;
        }
    }
    return $spaceIds;
...
function _stattic_runtime_job_step_tier_demote(string $privateRoot, array $job): array
{
    ...
    // step just takes the next N still-unmarked blobs.
    $targets = [];
    foreach ($blobs as $sha => $blob) {
        if ($blob['stat'] === null || $blob['demoted_at'] !== null || !isset($live[$sha])) {
            continue;
        }
    ...
    $total = $done + count($targets);

    $chunk = array_slice($targets, 0, STATTIC_TIER_DEMOTE_CHUNK);
    if ($chunk === []) {
        _stattic_runtime_append_journal($privateRoot, [
    ...
    $released = _stattic_space_write_lock_with(
        ...
        STATTIC_LOCK_WAIT,
        null,
        static function () use ($chunk, $blobs): array {
            $released = [];
            foreach ($chunk as $sha) {
                $path = $blobs[$sha]['path'];
                $size = (int) ($blobs[$sha]['stat']['size'] ?? 0);
                _stattic_storage_blob_demote_mark($path, ['reason' => 'demote', 'bytes' => $size]);
                if (is_file($path) && !@unlink($path)) {
...
{
    $live = [];
    foreach (glob(_stattic_space_root($privateRoot, $spaceId) . '/routes/*.json') ?: [] as $path) {
        if (!is_string($path)) {
            continue;
        }
