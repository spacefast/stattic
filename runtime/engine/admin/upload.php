...
function _stattic_runtime_publish_session_replace(string $privateRoot, string $spaceId, string $uploadId, callable $change): array
{
    ...
    $updated = _stattic_record_store_mutate($store, $uploadId, static function (?array $session) use ($privateRoot, $spaceId, $uploadId, $store, $change): array {
        ...
        // Pin first: a newly-bound URL-fetch digest must be protected before it
        // becomes an accepted CAS object.
...
function _stattic_runtime_publish_session_accept_many(string $privateRoot, string $spaceId, string $uploadId, array $shas): array
{
    return _stattic_runtime_publish_session_replace($privateRoot, $spaceId, $uploadId, static function (array $session) use ($shas): array {
        ...
        foreach ((is_array($session['manifest'] ?? null) ? $session['manifest'] : []) as $entry) {
            ...
            $declaredSizes[strtolower($entry['sha256'])] = (int) ($entry['size'] ?? -1);
        }
...
{
    if (_stattic_runtime_bearer_token_from_request() === null) {
...
{
    _stattic_problem_response(405, 'runtime_upload_operation_not_supported', 'Runtime upload only supports publish-session blob and URL-fetch operations.', [], ['Allow' => $allowedMethod]);
}
...
function _stattic_runtime_materialize_lazy_upload_session(string $privateRoot, string $uploadId, array $expected, array $descriptor, bool $includeRetained): array
{
    ...
    $record = [
        ...
        // Same rule as create_version: a reusable version with no list retains
        // everything, materialized from the prior catalog at finalize.
        'retain_all' => $reusableVersionId !== null && $retained === [],
...
function _stattic_runtime_upload_blobs_have(string $privateRoot, string $spaceId, array $claims): void
{
    ...
    foreach ($shas as $sha) {
        ...
        $sha = strtolower($sha);
        if (!_stattic_runtime_blob_has($privateRoot, (string) $session['space_id'], $sha)) {
            $missing[] = $sha;
        } elseif (array_key_exists($sha, $declaredSizes)) {
            $accepted[$sha] = $declaredSizes[$sha];
        }
    }
    if ($accepted !== []) {
...
function _stattic_runtime_upload_blob(string $privateRoot, string $spaceId, string $sha, array $claims): void
{
    ...
    $declaredSize = null;
    foreach ((is_array($session['manifest'] ?? null) ? $session['manifest'] : []) as $entry) {
        if (is_array($entry) && is_string($entry['sha256'] ?? null) && hash_equals(strtolower($entry['sha256']), $sha)) {
            $declaredSize = (int) ($entry['size'] ?? -1);
            break;
        }
    }
    ...
    $streamed = _stattic_runtime_blob_stage_stream($privateRoot, _stattic_binary_body_stream(), $declaredSize);
