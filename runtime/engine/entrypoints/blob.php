...
function _stattic_blob_gate_serve(string $privateRoot, string $requestMethod, string $requestPath): never
{
    ...
    $headers = [
        ...
        'ETag' => '"' . $sha256 . '"',
        // In THIS array, beside Content-Disposition, because only what
        // _stattic_send_server_file() hands over survives the X-Accel hop — the
        // dashboard reads gate bytes with fetch(), so an ACAO stripped at the
        // accel boundary would break every in-app preview.
