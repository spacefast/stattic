 ...
 * carries `{paths: string[], tags: string[]}`, and gets a 202: accepted into
 * the durable purge queue, not confirmed by the edge — the same queue and
 * provider bridge every management mutation already drains through
 * (shared/purge.php).
 *
...
// A purge body is a list of paths and tags, never content; anything above this
// is a broken caller, not a big purge.
const STATTIC_FUNCTIONS_PURGE_MAX_BODY_BYTES = 65536;

// Tags are opaque worker-side labels; the caps only bound the request shape.
const STATTIC_FUNCTIONS_PURGE_TAG_MAX = 64;
const STATTIC_FUNCTIONS_PURGE_TAG_MAX_LENGTH = 256;
...
/** @return list<string> deduplicated printable tags, invalid entries dropped. */
function _stattic_functions_purge_tag_list(mixed $raw): array
{
    $tags = [];
    foreach (is_array($raw) ? $raw : [] as $tag) {
        if (
            !is_string($tag)
            || $tag === ''
            || strlen($tag) > STATTIC_FUNCTIONS_PURGE_TAG_MAX_LENGTH
            || preg_match('/[\x00-\x1f\x7f]/', $tag) === 1
        ) {
            continue;
        }
        $tags[$tag] = true;
        if (count($tags) >= STATTIC_FUNCTIONS_PURGE_TAG_MAX) {
            break;
        }
    }
...
function _stattic_functions_purge_serve(string $privateRoot, string $spaceId, string $versionRoot, string $requestMethod): void
{
    ...
    if (!is_array($decoded)) {
        _stattic_problem_response(422, 'purge_invalid_body', 'Purge body must be a JSON object with paths and tags arrays.', [], ['Cache-Control' => 'no-store']);
    }
    $paths = _stattic_runtime_purge_path_list($decoded['paths'] ?? null);
    $tags = _stattic_functions_purge_tag_list($decoded['tags'] ?? null);
    if ($paths === [] && $tags === []) {
        _stattic_problem_response(422, 'purge_empty', 'Purge names no valid paths or tags.', [], ['Cache-Control' => 'no-store']);
    }

    $hostnames = _stattic_functions_purge_hostnames($privateRoot, $spaceId);
    $signature = hash('sha256', (string) json_encode([$hostnames, $paths, $tags], JSON_UNESCAPED_SLASHES));
    ...
    if (($decision['verdict'] ?? null) === 'accepted' && $hostnames !== []) {
        _stattic_runtime_purge_enqueue($privateRoot, [
            'hostnames' => $hostnames,
            // A tag names a group only the worker's own cache understands; the
            // provider purges URLs and domains, so any tag widens the record to
            // the domain scope an empty path set derives (shared/purge.php).
            'paths' => $tags === [] ? $paths : [],
        ...
        // The kick pays a CLI-subprocess bootstrap; it runs post-response (the
