...
function _stattic_php_functions_serve(array $context, array $action, string $requestPath): never
{
    ...
    try {
        ...
        error_log(sprintf(
            'spacefast php function containment failed type=%s message=%s',
 ...
 *   ]);
 *
 * This helper uses the complete `SpamSubmission` contract
 * (packages/common/src/contracts/runtime-services.ts): `content` is required,
 * `type` defaults to `comment`, `userIp`, `userAgent` and `referrer` default to
 * this request's own evidence (below), and `permalink`, `authorName`,
...
{
    $verdict = _stattic_php_functions_service_call(
        'spam',
        'check',
        _stattic_php_functions_spam_payload(_stattic_php_functions_spam_request_defaults($submission))
    );
...
/**
 * File a correction: a submission the verdict let through was spam after all.
 *
 ...
 * @param array<string,mixed> $submission
 */
function sf_report_spam(array $submission): void
{
    _stattic_php_functions_service_call('spam', 'report_spam', _stattic_php_functions_spam_payload($submission));
}
 ...
 * @param array<string,mixed> $submission
 */
function sf_report_ham(array $submission): void
{
    _stattic_php_functions_service_call('spam', 'report_ham', _stattic_php_functions_spam_payload($submission));
}
 ...
 * @return array<string,mixed>
 */
function _stattic_php_functions_spam_request_defaults(array $submission): array
{
    $state = &_stattic_php_functions_state();
    $bundle = is_array($state['services']) ? $state['services'] : [];
    $trustedIp = is_string($bundle['visitorIp'] ?? null) ? $bundle['visitorIp'] : '';
    $explicitIp = is_string($submission['userIp'] ?? null) ? trim($submission['userIp']) : '';
    if ($explicitIp === '' && $trustedIp !== '') {
        $submission['userIp'] = $trustedIp;
    }
    foreach (['userAgent' => 'HTTP_USER_AGENT', 'referrer' => 'HTTP_REFERER'] as $field => $header) {
        $explicit = is_string($submission[$field] ?? null) ? $submission[$field] : '';
        if ($explicit === '' && is_string($_SERVER[$header] ?? null) && $_SERVER[$header] !== '') {
            $submission[$field] = $_SERVER[$header];
        }
    }
