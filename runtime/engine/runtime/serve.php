...
function _stattic_v4_dispatch_action(array $context, array $action, array $entry, string $requestPath): void
{
    ...
    if ($type === STATTIC_RUNTIME_RESPONSE_ACTION_ZERO || $type === STATTIC_RUNTIME_RESPONSE_ACTION_FUNCTION) {
        _stattic_acquire_zero_admission((string) $context['private_root'], $serving);
...
function _stattic_v4_rule_section(?array $rulesEntry, string $section): ?array
{
    ...
    return ['exact' => $rules['exact'] ?? [], 'pattern' => $rules['pattern'] ?? []];
}
...
function _stattic_v4_dispatch_pattern_routes(array $context, string $requestPath, string $requestMethod, string $requestUri): void
{
    ...
    if (is_file($versionDir . '/zero/routes.php')) {
        ...
        $result = _stattic_resolve_zero_route_action($versionRoot, ['zero_routes' => true], $lookup, $requestMethod);
        if (!empty($result['method_not_allowed'])) {
            _stattic_render_method_not_allowed_lazy();
        }
        if (is_array($result['action'] ?? null)) {
            _stattic_acquire_zero_admission((string) $context['private_root'], $context['serving']);
            require_once __DIR__ . '/zero.php';
            _stattic_invoke_zero(
                $result['action'],
                $versionRoot,
    ...
    if (_stattic_version_has_functions($versionRoot)) {
        ...
        // A path the table claims at other methods is a 405 here, never a fall
        // through to the SPA index or a 404: the route exists, the verb does not.
...
// One admission slot per request: the access, Zero and static-stream lanes all
// reserve through these, before any matching rule (bcrypt included) can render
// a decision.
...
function _stattic_acquire_storage_admission(string $privateRoot, array $serving): void
{
    if (!empty($GLOBALS['SPACEFAST_ADMISSION_ACQUIRED'])) {
        return;
    }
    require_once __DIR__ . '/../shared/admission.php';
    _stattic_admission_acquire_or_shed($privateRoot, $serving, 'storage');
    $GLOBALS['SPACEFAST_ADMISSION_ACQUIRED'] = true;
    _stattic_admission_test_hold_if_requested();
}

function _stattic_acquire_zero_admission(string $privateRoot, array $serving): void
{
    if (!empty($GLOBALS['SPACEFAST_ADMISSION_ACQUIRED'])) {
        return;
    }
...
function _stattic_dispatch_storage(
    ...
    _stattic_acquire_storage_admission($privateRoot, $serving);
    require_once __DIR__ . '/storage.php';
...
{
    if ($bytes < STATTIC_STATIC_STREAM_ADMISSION_BYTES || !empty($GLOBALS['SPACEFAST_ADMISSION_ACQUIRED'])) {
        return;
    }
    require_once __DIR__ . '/../shared/admission.php';
    _stattic_admission_acquire_or_shed(
        (string) $context['private_root'],
        is_array($context['serving']) ? $context['serving'] : [],
        'static_stream'
    );
