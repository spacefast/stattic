...
// the edge answers conditionals off its own HIT (§16, C19). ETag and
// Last-Modified are still EMITTED per entry — they are the edge's validators.
...
function _stattic_serve_request(string $privateRoot, string $requestMethod, string $requestUri, string $requestPath, string $requestHost): void
{
            ...
            _stattic_serve_page('collab', [
                ...
                'headers' => [
                    'Cache-Control' => $privateCache
                        ? STATTIC_CACHE_CONTROL_PROTECTED
    ...
    if (is_array($entry)) {
        ...
        $bypassCapable = _stattic_v4_functions_static_bypass_capable(
            $versionRoot,
            $requestPath,
            $requestMethod,
        );
