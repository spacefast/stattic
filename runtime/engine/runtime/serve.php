...
function _stattic_serve_request(string $privateRoot, string $requestMethod, string $requestUri, string $requestPath, string $requestHost): void
{
    ...
    $routes = _sf_pointer_read('routes', $privateRoot . '/routes/current.json');
    ...
    $space = $spaceId === ''
        ? null
        : _sf_pointer_read('space:' . $spaceId, $privateRoot . '/spaces/' . $spaceId . '/space.json');
    ...
    // a stronger state, is a denial and never an open space.
    ...
    if (is_array($entry)) {
        _stattic_v4_send_entry($sendContext, $entry, $requestPath);
    }
...
function _stattic_v4_version_root_artifact(string $versionDir, ?string $routeName = null): ?array
{
    ...
    $pointer = _sf_pointer_read('version-root:' . hash('sha256', $versionDir), $versionDir . '/root.json');
