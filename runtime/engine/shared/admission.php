...
// The access lane's one slot, charged at most once per request. Callers reach
// this from wherever the "this request needs identity work" verdict is known,
// so the once-guard and the OPTIONS exemption live here instead of at each of
// them.
function _stattic_admission_acquire_access_lane(string $privateRoot, array $serving): void
{
    if (!empty($GLOBALS['SPACEFAST_ADMISSION_ACQUIRED']) || _stattic_runtime_request_method() === 'OPTIONS') {
        return;
    }
