...
const STATTIC_TOMBSTONE_VARIANTS = [
    'tombstone-csam' => ['template_id' => 'undeployed', 'status' => 503, 'body' => "This site has not been deployed yet.\n", 'robots' => false, 'cache_control' => STATTIC_CACHE_CONTROL_NO_STORE],
    ...
    'tombstone-suspended' => ['template_id' => 'tombstone-suspended', 'status' => 402, 'body' => "This site is suspended.\n", 'robots' => true, 'cache_control' => STATTIC_DEFAULT_EDGE_CACHE_CONTROL],
    'tombstone-generic' => ['template_id' => 'tombstone-generic', 'status' => 404, 'body' => "This site is no longer available.\n", 'robots' => true, 'cache_control' => STATTIC_DEFAULT_EDGE_CACHE_CONTROL],
];
...
/**
 * @return array{entry: array, routes: array}|null
 */
function _stattic_v4_host_lookup(string $privateRoot, array $routes, string $requestHost): ?array
{
    if ($requestHost === '') {
        return null;
    }
...
function _stattic_v4_overlay(string $privateRoot, string $spaceId, mixed $space): ?array
{
...
function _stattic_v4_include_artifact(string $path): mixed
{
    static $cache = [];
    ...
    $loaded = @include $path;
    return $cache[$path] = is_array($loaded) ? $loaded : null;
}
...
function _stattic_v4_legacy_serving(string $spaceId, ?string $versionId, array $hostEntry, array $overlay): array
{
    ...
    return [
        ...
        'anonymous_expires_at' => is_string($overlay['anonymous_expires_at'] ?? null)
            ? $overlay['anonymous_expires_at']
            : null,
