...
// Platform page fonts load from shared origins — Recoleta (commercial, never
// shipped in the engine) from wordpress.com's font CDN, Haskoy from
// spacefast.com (apps/www/public/assets/fonts, served immutable with CORS) —
// so every space hostname reuses one warm browser-cache entry instead of a
// cold per-space fetch. Mirrors packages/common/src/utils/page-fonts.ts.
// url => [family, weight, preload]; emission order is CSS order, and preload
// marks the faces the pages actually render (Recoleta 400 headings + Haskoy).
const STATTIC_PLATFORM_PAGE_FONTS = [
    ...
    'https://spacefast.com/assets/fonts/haskoy-latin-variable.woff2' => ['Haskoy', '100 900', true],
];
    ...
    register_shutdown_function(static function (): void {
        ...
        while ($queue !== []) {
            ...
            try {
                ...
                // Post-response: the visitor already has their bytes.
            }
        }
    });
}
...
function _stattic_dashboard_origin(): string
{
    ...
    $configured = _stattic_config_value('SPACEFAST_DASHBOARD_ORIGIN');
    $parts = $configured === '' ? false : parse_url($configured);
...
function _stattic_runtime_upload_api_route_path(string $requestPath): ?string
{
    ...
    if ($op === 'fetch') {
        ...
        if ($encodedPath === null || $encodedPath === '') {
            return '/' . rawurlencode($uploadId) . '/fetch/files/';
        }
...
{
    $normalizedPrefix = '/' . trim($prefix === '' ? '/' : $prefix, '/');
    if ($normalizedPrefix === '//') {
        $normalizedPrefix = '/';
    }
...
function _stattic_append_current_query_to_url(string $target): string
{
    ...
    // this response. Carry every harmless pair across a canonical redirect,
    // but never send the durable Link secret to the next URL. Before exchange
    // (for example a host redirect) the cookie marker is false, so the token
    // still reaches the host that must redeem it.
    if (_stattic_access_query_token_present() && _stattic_identity_cookie_mutated()) {
