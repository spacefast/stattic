...
function _stattic_runtime_php_cli_binary(): string
{
    ...
    // No is_file probe: FPM's open_basedir confines PHP file checks to the
    // site tree, so probing /usr/local/.../bin/php reports false even though
    // the binary exists and execs fine — which silently downgraded this to a
    // bare 'php' that an env-carrying proc_open (execve, no PATH search) can
    // never resolve. PHP_BINDIR is trusted; a wrong value fails the spawn
    // loudly, exactly like the bare fallback did.
