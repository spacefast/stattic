...
// with _stattic_runtime_with_subprocess_env, never with an env array.
 ...
 */
function _stattic_runtime_with_subprocess_env(array $set, array $unset, callable $fn): mixed
{
    $touched = [];
    foreach ([...array_keys($set), ...$unset] as $name) {
        if (!array_key_exists($name, $touched)) {
            $previous = getenv($name);
            $touched[$name] = $previous === false ? null : $previous;
        }
    }
    ...
    try {
        ...
        foreach ($touched as $name => $previous) {
            if ($previous === null) {
                putenv($name);
            } else {
