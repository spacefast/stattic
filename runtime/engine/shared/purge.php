...
function _stattic_runtime_purge_kick_detached(string $privateRoot): bool
{
    ...
    return _stattic_runtime_start_detached_subprocess(
        ...
        // Inherit the provider environment. wp-load.php and the Edge Cache
        // bridge depend on values this runtime cannot enumerate safely.
        null
    );
}
...
function _stattic_runtime_purge_now(string $privateRoot, array $input): array
{
    ...
    if (function_exists('fastcgi_finish_request')) {
        ...
        _stattic_defer(static function () use ($privateRoot): void {
            _stattic_runtime_purge_kick_detached($privateRoot);
        });
        $status = 'queued';
    } else {
