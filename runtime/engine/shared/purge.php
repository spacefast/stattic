...
// (directory, extensionless, HTML, JSON, XML) a single POP accepting a URI
// invalidation does not prove every visitor-facing key converged, so any such
// path widens the activation to a domain purge — a fail-closed default, not a
// measured failure: both recorded live probes (2026-08-07 and 2026-08-19) saw
// a document URI purge converge sub-second at the observing POP, with an
// un-purged sibling URL still serving its stored copy. The 300 URL ceiling
// remains the provider's scoped-call limit.
 ...
 * root so the visitor lane can call it without the management readers. A
 * space no hostname has ever served is unaddressable at the provider; there
 * is no edge entry to drop, and 'none' says so honestly.
 *
 * @return array{status: string, mode: string, urls?: int}
 */
