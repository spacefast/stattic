...
function _stattic_runtime_purge_kick(string $privateRoot): array
{
    ...
    $result = _stattic_runtime_run_subprocess(
        ...
        _stattic_runtime_purge_child_env(),
        null,
        null,
...
function _stattic_runtime_purge_kick_detached(string $privateRoot): bool
{
    ...
    return _stattic_runtime_start_detached_subprocess(
        [
            _stattic_runtime_php_cli_binary(),
            $entrypoint,
            '--private-root=' . $privateRoot,
...
/**
 * The provider environment minus the FastCGI request variables. wp-config.php
 * and the Edge Cache bridge need the host-supplied values, so the child gets a
 * copy of everything — except the per-request keys wp.cloud's FPM exports into
 * the process environment. Leaked into a CLI child they land in $_SERVER, and
 * the loader then classifies the purge worker as a simulated request and
 * serves-then-exits it (verified live: REQUEST_METHOD in the child env
 * reproduces the hijack; without it the same worker drains the queue).
 */
function _stattic_runtime_purge_child_env(): array
{
    $env = getenv();
