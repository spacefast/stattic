...
// Read, never include: this program republishes the pointer and then reads it
// back within the same process, and an `include` under opcache's default
// revalidate_freq can answer from the superseded compiled file.
function active_release_pointer_target(string $installRoot): ?string
{
    foreach (['/active-release.php' => true, '/active-release' => false] as $name => $php) {
...
// The pointer artifact is one statement wide by construction (see
// publish_active_release), so its target is read back by parsing that one form
...
// This tiny PHP file is the only mutable release state, and every request
// resolves the engine through it — so it is a `return` statement the loader
// includes from opcache, not a data file each request re-reads. rename()
// replaces it atomically, and unlike a symlink its selected target is not held
// across requests by PHP-FPM's realpath cache.
function publish_active_release(string $installRoot, string $target): bool
{
    if (preg_match('#^releases/[A-Za-z0-9._-]+$#', $target) !== 1) {
        return false;
    }
    $pointer = $installRoot . '/active-release.php';
    ...
    @unlink($temporary);
    if (
        @file_put_contents($temporary, "<?php return '" . $target . "';\n", LOCK_EX) === false
    ...
    clearstatcache(true, $pointer);
    // Best effort by design: this runs in a CLI process whose opcache is not
    // the serving pool's, and PHP-FPM revalidates the renamed file by
    // timestamp. A failure here is not a failed publication.
    if (function_exists('opcache_invalidate')) {
        @opcache_invalidate($pointer, true);
    }
    // The plain-text pointer only exists on a box that last activated before
    // this release. Aliases are installed before publication, so by the time
    // this runs the loader that reads the plain file is already replaced.
    @unlink($installRoot . '/active-release');
    return active_release_pointer_target($installRoot) === $target;
}

function remove_active_release_pointer(string $installRoot): bool
{
    $removed = @unlink($installRoot . '/active-release.php');
    @unlink($installRoot . '/active-release');
    return $removed;
}
