...
function handoffToken(host: string, spaceId: string, authorities: string[]): string {
...
test("a share token on the URL returns that page in the same response", async () => {
  ...
  // ordinary query state, but the session cookie replaces the Link secret on
  // the next URL.
  const canonical = await get(runtime, LANES_HOST, `/docs?view=compact&__=${LINK_TOKEN}&page=2`);
  expect(canonical.status).toBe(308);
  expect(canonical.headers.get("location")).toBe("/docs/?view=compact&page=2");
...
test("an invalid access URL never serves customer content", async () => {
  ...
  const before = exchangeRequests.length;
  for (const shape of ["notAValidShareToken00", "sfz_someOtherThing0000", "", "a.b.c"]) {
    const refused = await get(runtime, LANES_HOST, `/docs/?__=${shape}`);
