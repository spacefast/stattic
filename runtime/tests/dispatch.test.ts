...
test("binary endpoints and malformed envelopes are rejected", async () => {
  // The accel-probe row writes its own raw response (headers, no JSON body), so
  // it carries binary => true and is rejected before auth on the
  // JSON-envelope-only dispatch transport.
  const binaryRoute = await dispatch({
    method: "POST",
    path: runtimeHttpPath("/__spacefast/api.php/probe/headers"),
    authorization: `Bearer ${managementToken("probe_headers")}`,
  });
