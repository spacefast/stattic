...
test("a finalize failure moves an existing version aside and restores it", async () => {
  const spaceId = "spc_native_replace";
  const versionId = "ver_native_replace_1";
  const replacementFiles = { "index.html": "<h1>replacement</h1>\n" };

  const session = await createDeclaredSession(rt, spaceId, versionId, replacementFiles);
  await uploadSessionBlobs(rt, session, replacementFiles);

  // Model a directory left at this immutable version coordinate before the
  // upload session commits. Finalize must move it aside while the compiler
  // runs so a failed attempt can restore it intact.
  const root = versionRoot(rt, spaceId, versionId);
  mkdirSync(root, { recursive: true });
  const partialMarker = path.join(root, "partial-marker");
  writeFileSync(partialMarker, "prior partial finalize\n");
  ...
  try {
    const failed = await finalize(rt, spaceId, versionId, { upload_id: session.uploadId });
