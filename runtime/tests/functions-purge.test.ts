...
function purgeQueueRecords(): Array<Record<string, unknown>> {
  const root = storagePath(rt, "runtime", "purges");
  if (!existsSync(root)) return [];
  return readdirSync(root)
    .filter((name) => name.endsWith(".json"))
    .map(
      (name) => JSON.parse(readFileSync(path.join(root, name), "utf8")) as Record<string, unknown>,
    );
}
...
test("the contract: POST only, bearer in sf-purge-token, 202 {accepted:true}", async () => {
  ...
  const records = purgeQueueRecords().filter((record) => record.reason === "functions_purge");
...
test("identical sets coalesce; distinct sets spend the per-space quota", async () => {
  writePurgeToken(PURGE_TOKEN);
  const before = purgeQueueRecords().filter((r) => r.reason === "functions_purge").length;
  ...
  const afterCoalesce = purgeQueueRecords().filter((r) => r.reason === "functions_purge").length;
...
test("a tag purge widens to the domain scope the provider can actually honor", async () => {
  writePurgeToken(PURGE_TOKEN);
  ...
  const record = purgeQueueRecords()
    .filter((r) => r.reason === "functions_purge")
    .at(-1);
  // A tag names a group only the worker's cache understands; the provider
  // purges URLs and domains, so the record derives the domain scope.
  expect(record?.scope).toBe("domain");
});
