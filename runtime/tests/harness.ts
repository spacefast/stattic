...
//
// v4 shapes this file speaks (internal-docs/runtime-rewrite-contracts-2026-08-07):
 ...
 * use the identical convention in v4 — the upload lane's op=/upload_id=/part
 * spellings are gone with the path-addressed upload.
 *
 * Note: the upload surface rejects EVERY query parameter except `route`
 * (`_stattic_runtime_non_platform_upload_query`), so extra params passed through
 * here are only meaningful on the management surface (or as a deliberate 405
 * probe).
 */
...
export async function startRuntime(options: RuntimeOptions = {}): Promise<Runtime> {
  ...
  const server = spawn(PHP_BINARY, phpArgs, {
    cwd: root,
...
// ---------------------------------------------------------------------------
// Publish: declared session -> content-addressed blobs -> finalize (contracts §9)
// ---------------------------------------------------------------------------
 ...
 * pin (D92). Blobs are uploaded afterwards, addressed by sha and never by path.
 */
