...
const HTML_OPEN = "public, s-maxage=31536000, max-age=0, must-revalidate";
...
// row here fails for its own reason: an HTML page is always revalidated by the
// browser and held at the edge until publish purges it; a name that already
// carries a content hash is frozen; a date, a version number and an ordinary
// asset name are not hashes and must revalidate.
