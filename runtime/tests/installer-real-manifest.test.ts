...
async function installFromShippedManifest(): Promise<RealManifestInstall> {
  ...
  const child = Bun.spawn({
    cmd: ["php", "-d", "auto_prepend_file=", residentInstaller, `${baseUrl}/engine.zip`],
    stdout: "pipe",
    stderr: "pipe",
    env: {
      ...process.env,
...
test("the shipped manifest installs executable engine bytes without owning the resident installer", async () => {
  ...
  const installedBinary = path.join(install.publicRoot, ".stattic/bin/stattic-runtime");
  expect(statSync(installedBinary).mode & 0o111).not.toBe(0);
  expect(statSync(path.join(install.publicRoot, ".stattic/engine/init.php")).isFile()).toBe(true);
