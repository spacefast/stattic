...
// Spam evidence, all three seams in one request. The check names no userIp, so
// only the dispatch-time trusted visitor IP can carry it past validation to the
// broker's own refusal (no Akismet key). The corrections are a moderation flow:
// the first carries its complete evidence and reaches the same refusal; the
// second omits userIp and must fail validation HERE — a correction gets no
// request defaults, because this request's visitor is the moderator, not the
// spammer whose evidence is being filed.
...
beforeAll(async () => {
  ...
  rt = await startRuntime({
    env: {
      ...
      // The ingress-owned visitor identity, exactly as the Zero suite carries
      // it. Environment-sourced, so the scrub removes it before the handler
      // runs — the probe below still proving no SPACEFAST_* name survives is
      // what forces the dispatch-time capture sf_spam's default rides on.
      SPACEFAST_VISITOR_IP: "203.0.113.77",
    },
    // PCOV corrupts its native allocator state after several requests whose
    // handler tightens open_basedir (`munmap_chunk(): invalid pointer` on the
    // Ubuntu runner). This suite proves the jail with the actual PHP runtime;
    // the other runtime suites continue to own PHP line coverage.
...
test("spam evidence: the trusted visitor IP fills a check's userIp; corrections name their own", async () => {
  ...
  expect(((await response.json()) as { codes?: string[] }).codes).toEqual([
    // Past validation without a caller-named userIp — only the trusted
    // visitor IP captured at dispatch can have carried it to the broker.
