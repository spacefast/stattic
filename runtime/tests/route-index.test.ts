...
test("activating a new version swaps the space overlay and leaves every route shard addressed the same", async () => {
  ...
  try {
    ...
    // are identical, so their content addresses are identical — the v4 shape of
    // "untouched shards are not rebuilt".
...
async function maintenanceTick(rt: Runtime, spaceId: string): Promise<void> {
  ...
  const created = await apiJson<{ job: { id: string } }>(
    ...
    "create_engine_job",
    claims,
    { type: "maintenance_tick", payload: {}, idempotency_key: "idem-shard-gc" },
    201,
  );
