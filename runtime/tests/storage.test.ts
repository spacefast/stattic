...
test("an anonymous commenter session is the upload grant — budgeted, and only where Comments is on", async () => {
  ...
  const record = JSON.parse(
    readFileSync(
      path.join(storagePath(strict, "spaces", ANON_SPACE, "uploads", "objects"), `${object.id}.json`),
      "utf8",
    ),
  ...
  expect(
    (await get(strict, ANON_HOST, `/storage/${object.id}`, { method: "DELETE", headers: { cookie } }))
      .status,
  ).toBe(204);
