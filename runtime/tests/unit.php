...
// The spawn env scope: FastCGI request variables are unset around the spawn (a
// leaked REQUEST_METHOD makes the loader hijack the CLI worker), extra values
// are exported for the child, host-supplied values stay inherited, and every
// touched variable is restored afterwards.
putenv('REQUEST_METHOD=POST');
putenv('HTTP_HOST=example.test');
putenv('QUERY_STRING=a=1');
putenv('SPACEEFAST_UNRELATED=untouched');
$requestNames = _stattic_runtime_purge_request_env_names();
check(in_array('REQUEST_METHOD', $requestNames, true), 'request env names: lists REQUEST_METHOD');
check(in_array('HTTP_HOST', $requestNames, true), 'request env names: lists HTTP_ vars');
check(in_array('QUERY_STRING', $requestNames, true), 'request env names: lists QUERY_STRING');
check(!in_array('SPACEEFAST_UNRELATED', $requestNames, true), 'request env names: leaves host-supplied values alone');
...
check($insideSeen['request'] === false, 'subprocess env scope: request vars unset inside');
...
check(getenv('REQUEST_METHOD') === 'POST', 'subprocess env scope: request vars restored after');
