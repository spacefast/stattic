...
// caching, so the lane declares no policy and Cache-Control relays.
$fxPublicPolicy = _stattic_cache_policy(['private' => false, 'public' => null]);
...
// metadata is replaced rather than relayed.
$fxPrivatePolicy = _stattic_cache_policy(['private' => true, 'public' => null]);
...
check(sf_test_apcu_has($catalogKey), 'reading a catalog caches it pool-wide');
check(
    sf_test_apcu_has($blobShasKey),
