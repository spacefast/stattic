...
check(_stattic_join_request_path('/design', '/') === '/design', 'path join keeps a non-slash prefix');
check(
    _stattic_join_request_path('/design/', '/') === '/design/',
    'path join keeps a terminal slash at the route root'
);
check(
    _stattic_join_request_path('/design/', '/asset.js') === '/design/asset.js',
    'path join appends below a slash prefix once'
);
check(
    _stattic_append_path_to_url('https://origin.example/design/', '/') === 'https://origin.example/design/',
    'URL path join keeps an upstream terminal slash',
);

// --- Verified absence: directory evidence, never a failed stat ----------------------

$absenceRoot = realpath(sys_get_temp_dir()) . '/sf-absence-' . bin2hex(random_bytes(6));
mkdir($absenceRoot, 0o777, true);
file_put_contents($absenceRoot . '/present.json', "{}\n");
mkdir($absenceRoot . '/wrong-type.json');
check(
    _sf_path_verifiably_absent($absenceRoot . '/missing.json'),
    'verified absence: a readable parent proves a missing filename'
);
check(
    _sf_path_verifiably_absent($absenceRoot . '/missing-parent/current.json'),
    'verified absence: a readable ancestor proves a missing parent component'
);
check(
    !_sf_path_verifiably_absent($absenceRoot . '/present.json'),
    'verified absence: an existing file is never absent'
);
check(
...
for ($p = 0; $p < 64; $p++) {
    ...
    for ($j = 0; $j < 190; $j++) {
        touch($gcBlobsRoot . '/' . $gcPrefix . '/' . $gcPrefix . sprintf('%062x', $p * 1000 + $j));
        $gcBlobCount += 1;
    }
}
...
$gcResult = _stattic_tier_space_blob_gc($gcPrivateRoot, 'spc_gc_scale', time(), 0);
$gcPeakDelta = memory_get_peak_usage(false) - $gcMemBefore;
putenv('SPACEFAST_LOCAL_BLOB_GC_UNDECLARED_MIN_GRACE_SECONDS');
check(
    $gcResult['complete'] === true && $gcResult['deleted'] === $gcBlobCount,
    "blob gc at scale: one grace-0 pass collects all {$gcBlobCount} garbage blobs"
);
check(
    !is_file($gcBlobsRoot . '/00/00' . sprintf('%062x', 0)),
    'blob gc at scale: the collected bytes actually left the disk'
);
check(
 ...
 */
function stage_inbound_headers(array $headers): void
{
    ...
    foreach ($headers as $name => $value) {
        ...
        $_SERVER[in_array($upper, ['CONTENT_TYPE', 'CONTENT_LENGTH'], true) ? $upper : 'HTTP_' . $upper] = $value;
    }
}
...
check($brandDefaults['fonts'] === [], 'brand: the compiled-in document downloads no font');
...
$scopeIntentPath = _stattic_space_root($scopeRoot, 'spc_scope') . '/hostname-intent.json';
$scopeIntentSnapshot = _stattic_runtime_read_json_strict($scopeIntentPath);
$replacementRoutes = [$scopeIntent['routes'][1]];
chmod($scopeIntentPath, 0o000);
...
$storedReplacement = _stattic_runtime_read_json_strict($scopeIntentPath);
check(
    is_array($storedReplacement) && ($storedReplacement['routes'] ?? null) === $replacementRoutes,
    'activation stores hostname intent from its preflight snapshot without rereading the old document'
);
...
$insideSeen = _stattic_runtime_with_subprocess_env(
    ['SPACEFAST_TEST_EXPORTED' => 'yes'],
    ['SPACEFAST_TEST_UNSET_ME'],
    static fn (): array => [
        'unset' => getenv('SPACEFAST_TEST_UNSET_ME'),
        'exported' => getenv('SPACEFAST_TEST_EXPORTED'),
        'inherited' => getenv('SPACEEFAST_UNRELATED'),
        'path' => getenv('PATH'),
    ],
);
check($insideSeen['unset'] === false, 'subprocess env scope: unset names removed inside');
check($insideSeen['exported'] === 'yes', 'subprocess env scope: extras exported inside');
check($insideSeen['inherited'] === 'untouched', 'subprocess env scope: host values inherited');
check($insideSeen['path'] !== false, 'subprocess env scope: PATH survives');
check(getenv('SPACEFAST_TEST_UNSET_ME') === 'present', 'subprocess env scope: unset names restored after');
