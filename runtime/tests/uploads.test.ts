...
// `POST /spaces/{s}/blobs/have` and `PUT /spaces/{s}/blobs/{sha}` — plus the
// one surviving path-addressed product lane, URL fetch. Covered here: the
...
// URL fetch is the one path-addressed ingest route that survives §9 — it is a
// product feature, and the reason it needs its own egress guard.
...
test("the upload surface answers only its two blob routes", async () => {
  const content = "x";
