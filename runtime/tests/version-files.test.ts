...
//
// Deliberately NOT here, and not provable here: Range/206, 416 and conditional
// 304. `_stattic_send_server_file` hands the object to Nginx, which owns every
// one of those, and this fixture runs `php -S` where the accel hop does not
// exist (contracts §15/§16 — the platform never delivers Range to the origin).
// Asserting them against the PHP body fallback would prove the substitute, not
// the product. They belong to the credential-gated wp.cloud suite that drives
// the real edge.
...
test("a reusable version with no list republishes everything the runtime already holds", async () => {
  ...
  // No `files`, no `retained_files`: the caller names the version to carry
  // forward and the runtime materializes the path list from its own catalog.
  await deploy(rt, {
    spaceId: SPACE,
    versionId: reusedVersion,
    files: {},
    session: { reusableVersionId: VERSION },
    finalize: { variable_scopes: VARIABLE_SCOPES },
  });
