...
test("compiles zero_endpoints during finalize and invokes generated bytecode", async () => {
  ...
  // Derived from the Space's claim state, not from the artifact: this fixture is
  // claimed, so its handlers reach any public host. unit.php owns the
  // derivation itself and the runner owns what it does with the value.
  expect(envelope.context.egressScope).toBe("open");
...
test("rejects writes from a read handler before they reach MySQL", async () => {
  ...
  expect(await response.json()).toEqual({
    ...
    write: {
      ...
      message: "A Zero read or action handler cannot execute a database write.",
    },
  });
